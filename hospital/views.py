...
def home_view(request):
    try:
        ...
        doctor_count = MongoDoctor.objects.count()
        patient_count = MongoPatient.objects.count()
        appointment_count = MongoAppointment.objects.count()
        mongo_stats = {
    ...
    return render(request, 'hospital/index.html', 
                 {'mongo_connected': mongo_connection, 'mongo_stats': mongo_stats})
    ...
    if request.method=='POST':
        ...
        if userForm.is_valid() and doctorForm.is_valid():
            ...
            mongo_doctor = MongoDoctor(
                ...
                mobile=doctorForm.cleaned_data['mobile'],
                department=doctorForm.cleaned_data['department'],
                status=False
            )
            if 'profile_pic' in request.FILES:
                mongo_doctor.profile_pic = request.FILES['profile_pic'].name
            mongo_doctor.save()
    ...
    if request.method=='POST':
        ...
        if userForm.is_valid() and patientForm.is_valid():
            ...
            mongo_patient = MongoPatient(
                ...
                mobile=patientForm.cleaned_data['mobile'],
                symptoms=patientForm.cleaned_data['symptoms'],
...
def admin_add_appointment_view(request):
    ...
    if request.method=='POST':
        ...
        if appointmentForm.is_valid():
            ...
            # Save to MongoDB
            mongo_appointment = MongoAppointment(
                patient_id=str(request.POST.get('patientId')),
                doctor_id=str(request.POST.get('doctorId')),
                appointment_date=appointmentForm.cleaned_data['appointmentDate'],
                description=appointmentForm.cleaned_data['description'],
                status=True
            )
            mongo_appointment.save()
